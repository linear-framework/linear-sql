...
class PgNonTransactionalSpec extends BaseSpec with PostgresDataSource {

  override protected def beforeEach(): Unit = {
    super.beforeEach()
    execute("DROP TABLE IF EXISTS persons;")
    execute("CREATE TABLE persons(id BIGSERIAL PRIMARY KEY, name VARCHAR(60), age INTEGER)")
  }
  ...
  it can "update a record returning its auto-generated key" in {
    execute("INSERT INTO persons(name, age) VALUES ('Molly', 12)")
    val id =
      db
        .sql("UPDATE persons SET age = {age} WHERE name = {name}")
        .params("age" -> 13, "name" -> "Molly")
        .returningKey(_.getLong("id"))
        .execute()
  ...
  it can "update multiple records returning all auto-generated keys" in {
    execute("INSERT INTO persons(name, age) VALUES ('Teddy', 11)")
    execute("INSERT INTO persons(name, age) VALUES ('Henry', 12)")
    val ids =
      db
        .sql("UPDATE persons SET age = {age} WHERE name = {name}")
        .batch(
          List("age" -> 12, "name" -> "Teddy"),
          List("age" -> 13, "name" -> "Henry"),
          List("age" -> 14, "name" -> "Kathleen")
        )
        .returningKeys(_.getLong("id"))
        .execute()

    ids.length should be(2)
    ids.foreach(id => (id > 0) should be(true))
  }
